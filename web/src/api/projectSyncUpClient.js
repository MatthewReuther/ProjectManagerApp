...
export default class ProjectSyncUpClient extends BindingClass {
     ...
     * @param errorCallback (Optional) A function to execute if the call fails.
     * @returns The list of tasks on a project.
     */
    ...
    async updateProject(projectId, projectName, projectDescription, projectStatus, errorCallback) {
        try {
            const token = await this.getTokenOrThrow("Only authenticated users can update projects.");
            const response = await this.axiosClient.put(`project/${projectId}`, {
                projectId: projectId,
                projectName: projectName,
                projectDescription: projectDescription,
